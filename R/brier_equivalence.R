...
#'     It checks whether the models perform equivalently on a test set and
#'     returns various figures.
...
#'   \item{\code{crit_val}}{level-\eqn{\alpha} critical value for the test}
#' }
...
brier_equivalence <- function(model_a, model_b, test_data,
  ...
  bs_ac <- brier_score(test_data$y, pi_ac)
  bs_bc <- brier_score(test_data$y, pi_bc)
  b_ac <- (pi_ac - test_data$y)^2
  b_bc <- (pi_bc - test_data$y)^2
  d <- b_ac - b_bc
  ...
  return(list(brier_score_ac = bs_ac,
